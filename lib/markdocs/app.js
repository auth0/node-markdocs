...
App.prototype.get = function(request, response) {
  ...
  var doc = this._docsByUrl[url];
  var view = (doc.getMeta().layout || 'doc') + '.jade';

  var fill = function(doc, context) {
    context.url = url;
    context.meta = doc.getMeta();
    context.site = {};
    self.emit('prerender', request, response, doc);
    context.sections = doc.processSections(response.locals);
  };
